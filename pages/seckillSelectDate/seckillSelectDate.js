        ...
        if (dateList[i].id == curreInid) {
          ...
          for (let k = 0; k < days.length; k++) {
            if (days[k].day == dayIn) {
              ...
              days[k].inday = true;
              ++count
            } else if (days[k].day == dayOut) {
              days[k].class = days[k].class + ' finish';
              days[k].outday = true;
              ++count
            } else if (days[k].day >= dayIn && days[k].day <= dayOut) {
        ...
        if (dateList[j].month == monthIn) { //入住的开始月份
          ...
          for (let k = 0; k < days.length; k++) {
            if (days[k].day == dayIn) {
              ...
              days[k].inday = true;
              ++count
              console.log("上月active" + count)
            } else if (days[k].day >= dayIn) {
              days[k].class = days[k].class + ' bgitem';
              ++count
              console.log("上月bgitem" + count)
            }
          }
        } else { //入住跨月月份

          if (dateList[j].month == monthOut) { //离店最后的月份
            let days = dateList[j].days;
            for (let k = 0; k < days.length; k++) {
              if (days[k].day == dayOut) {
                ...
                days[k].outday = true;
                ++count
                console.log("下月finish" + count)
              } else if (days[k].day > 0 && days[k].day <= dayOut) {
                days[k].class = days[k].class + ' bgitem';
                ++count
                console.log("下月bgitem" + count)
              }
            }
          } else if ((dateList[j].year == yearIn && dateList[j].month > monthIn && yearIn != yearOut)
            || (dateList[j].year == yearOut && dateList[j].month < monthOut && yearIn != yearOut)
            || (yearIn == yearOut && dateList[j].month < monthOut && dateList[j].month > monthIn)) { //离店中间的月份
            let days = dateList[j].days;
            console.log(dateList[j].month)
            for (let k = 0; k < days.length; k++) {
              if (days[k].day > 0) {
                days[k].class = days[k].class + ' bgitem';
                ++count
                console.log("中月bgitem" + count)
              }
            }
          }
        }
